...
You are provided with a working sequential implementation of this integration in `trap-seq.c`. On the server, run this to make only it:
...
Notice what gets printed. These are fairly extensive print statements so that you can see what is happening. You will need to update these later. The important idea here is that we have used the function sine(x), which when integrated from 0 to &pi; should equal 2.0.
...
# Goal 1: create a working parallel version

Update `trap-omp.c` to produce correct multi-threaded code. Concentrate on the second of these 2 lines for the pragma that splits up the for loop:
...
You will need to decide what variables should be shared among the threads, which should be private to each thread, and which need *reduction* due to being shared by all threads. To help you decide, recall the opnenMP patternlet example for reduction. Also, the rule of thumb for a shared variable is that it isn't being changed by any threads. If any forked threads that are doing part of the loop work are updating variables, then that variable will need to be private *or* in a reduction.

When you correctly fix the private and reduction variables, you are fixing the potential **race conditions*.
...
Note that 268435455 is a power of 2 and the number of threads is increasing by multiples of 2. this divides the problem evenly between the threads and demonstrates that adding twice as many threads cuts the time almost in half, but not quite, from the previous run.
...
Note that the folder called other has some versions of the code with the fixes needed above and below in them already. If you have any trouble, refer to those.
...
The first thing to do is to have the two code files simply print out a time, followed by a tab. The lines you need to do this simply need to be uncommented.

1. For `trap-se.c`, there are two places to comment out the prints that you have seen so far, and place one simpler one in. 

    - Look for a TODO for Goal 2 on or near line 30. Comment the printf line following it.
    - Look for the TODO for goal 2 comments in the code near the end of the file at the end of main() that need to be commented. There is also one printf line below it that needs to be uncommented (this prints the time followed by a tab). 

2. For `trap-omp.c`, there are 2 different areas of the code that need to be updated.

    - Look for the first TODO for goal 2 near line 37, where you should comment out **only the printf lines**.
    - Look for another TODO for goal 2 near then end of the file and the end of main(), where you should comment out the original printf statements and uncomment a new simpler one that prints the time followed by a tab.

Try one of example of each, trap-seq and trap-omp, after making them to be sure that each seems to be printing only times, followed by a tab.
...
(say over 75%), we would consider the program to be **strongly scalable** *over the range of threads where this is the case*. We therefore often report a range of threads and a range of problem sizes for which our code exhibits strong scalability. 
